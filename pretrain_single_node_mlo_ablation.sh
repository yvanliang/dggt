...
usage() {
    cat <<'EOF'
用法：
  bash pretrain_single_node_mlo_ablation.sh
  bash pretrain_single_node_mlo_ablation.sh --resume CHECKPOINT EXPECTED_STEP WANDB_RUN_ID

Metric Layout Only（A4/MLO）单机训练：
  - 单机 8 GPU，每 GPU batch 1，gradient accumulation 8，global batch 64；
  - 无参数时从 step 0 开始全新训练，禁止从 Full、FSU、LOS 或 ISLO 分叉；
  - early map/actor raster stem 固定 off/off，late M/G/A context 固定 on/on/on；
  - scene units 固定 generated，world feedback 固定 full，其余训练合同保持 Full；
  - resume 只接受显式记录 metric_layout_only 的完整 MLO checkpoint；
  - resume 必须同时给出 checkpoint、期望 step 和原 W&B run id，W&B resume 固定为 must；
  - 训练、验证和 W&B 参数与两机 PPU MLO launcher 对齐。
EOF
}
...
# A4 is orthogonal to A1-A3: only the layout computation profile changes.
SCENE_UNITS_PROFILE="generated"
WORLD_FEEDBACK_PROFILE="full"
WORLD_FEEDBACK_EFFECTIVE="1/1/1"
LAYOUT_PATH_PROFILE="metric_layout_only"
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/scene_flow_pretrain_mlo_v6}"
LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-${PROJECT_ROOT}/logs/single_node_mlo_v6_launch}"
...
WANDB_NAME="${WANDB_NAME:-scene_flow_pretrain_waymo_gb64_mlo_v6}"
...
require_mlo_name() {
    ...
    if [[ ! "${basename}" =~ (^|[_-])mlo([_-]|$) ]]; then
        echo "[错误] ${name} 的 basename 必须包含独立的 mlo 标签，当前值：${value}" >&2
        exit 1
    fi
}

require_mlo_name LOG_DIR "${LOG_DIR}"
require_mlo_name LAUNCH_LOG_DIR "${LAUNCH_LOG_DIR}"
require_mlo_name WANDB_NAME "${WANDB_NAME}"
...
if [[ "${GRAD_ACCUM_STEPS}" != "8" ]]; then
    echo "[错误] A4 单机实验固定 GRAD_ACCUM_STEPS=8。" >&2
    exit 1
fi
...
if [[ -n "${RESUME_PATH}" && -z "${WANDB_RUN_ID}" ]]; then
    echo "[错误] MLO resume 必须提供原 W&B run id。" >&2
    exit 1
fi
...
if (( GLOBAL_BATCH_SIZE != EXPECTED_GLOBAL_BATCH_SIZE )); then
    echo "[错误] global batch size 配置不符合 A4 合同：" >&2
...
if missing:
    raise SystemExit(f"[错误] MLO resume 只接受完整训练 checkpoint；缺少 {missing}")
config = payload.get("scene_flow_config")
if not isinstance(config, dict):
    raise SystemExit("[错误] MLO checkpoint 缺少 scene_flow_config")
scene_units_profile = config.get("scene_units_profile", "generated")
if scene_units_profile != "generated":
    raise SystemExit(
        f"[错误] A4 scene_units_profile 必须为 generated，实际为 {scene_units_profile!r}"
    )
expected_injections = {
...
actual_injections = {name: config.get(name) for name in expected_injections}
if actual_injections != expected_injections:
    raise SystemExit(
        f"[错误] 明确拒绝非 MLO checkpoint；layout injections={actual_injections!r}"
    )
contract = payload.get("layout_path_profile_contract")
if not isinstance(contract, dict):
    raise SystemExit("[错误] MLO resume 要求显式 layout_path_profile_contract")
if (
    contract.get("schema") != "layout_path_profile_v1"
    or contract.get("requested_profile") != "metric_layout_only"
    or contract.get("resolved_profile") != "metric_layout_only"
    or contract.get("resolved_injections") != expected_injections
):
    raise SystemExit(f"[错误] MLO layout-path 合同不匹配：{contract!r}")
world_feedback = payload.get("world_feedback_contract")
if not isinstance(world_feedback, dict) or world_feedback.get("profile") != "full":
    raise SystemExit("[错误] A4 resume 要求 full world-feedback contract")
...
cd "${PROJECT_ROOT}"
build_train_args
LAUNCH_LOG="${LAUNCH_LOG_DIR}/single_node_mlo.log"

echo "============================================================"
echo "A4 Metric Layout Only 单机 8-GPU 预训练"
...
echo "layout path: requested_profile=${LAYOUT_PATH_PROFILE} resolved_profile=metric_layout_only early_map=false early_actor=false late_map=true late_actor=true late_appearance=true"
