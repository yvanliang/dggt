...
usage() {
    cat <<'EOF'
用法：
  bash /mnt/workspace/dggt/pretrain_ppu_one_node_los_dlc.sh

LOS 是 Latent-Only Supervision 的固定简称。此脚本不接受 setting 参数，
并固定使用 latent_only world-feedback profile：
  - PAI-DLC 固定使用 1 个 PPU 节点、每节点 16 张 PPU；
  - 每 PPU batch 1、gradient accumulation 4，global batch 64；
...
可覆盖环境变量：
  ...
  LAUNCH_LOG_DIR      单节点启动日志目录；basename 必须含独立的 los 标签
...
if [[ ! -r "${BASE_DLC_LAUNCHER}" ]]; then
    echo "[错误] 找不到公共 PPU 启动脚本：${BASE_DLC_LAUNCHER}" >&2
    exit 1
fi
...
export LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-${PROJECT_ROOT}/logs/ppu_dlc_one_node_los_v6_launch}"
