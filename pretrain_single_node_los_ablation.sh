...
usage() {
    cat <<'EOF'
用法：
  bash pretrain_single_node_los_ablation.sh
  bash pretrain_single_node_los_ablation.sh --resume CHECKPOINT EXPECTED_STEP WANDB_RUN_ID

Latent-Only Supervision（A2/LOS）单机训练：
  - 单机 8 GPU，每 GPU batch 1，gradient accumulation 8，global batch 64；
  - 无参数时从 step 0 开始全新训练，禁止从 Full 或 FSU checkpoint 分叉；
  - 只关闭 RGB、decoded-feature、frozen-head 三层 scene world feedback；
  - sky-view、sky-mask、gauge、layout、decoder、renderer 与 LPIPS 全部保留；
  - resume 只接受显式记录 generated + latent_only 的完整 LOS checkpoint；
  - resume 必须同时给出 checkpoint、期望 step 和原 W&B run id，W&B resume 固定为 must；
  - 除单机拓扑外，训练、验证与 W&B 参数对齐两机 PPU LOS launcher。
EOF
}
...
# A2 is orthogonal to A1: force generated scene units and latent-only world
# feedback even if the calling shell contains stale profile variables.
SCENE_UNITS_PROFILE="generated"
WORLD_FEEDBACK_PROFILE="latent_only"
WORLD_FEEDBACK_EFFECTIVE="0/0/0"
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/scene_flow_pretrain_los_v6}"
LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-${PROJECT_ROOT}/logs/single_node_los_v6_launch}"

# ============================================================
# Training contract for A2 single-node runs:
# 1 node × 8 GPU × 1 sample/GPU × 8 accumulation = global batch 64.
# ============================================================
...
WANDB_NAME="${WANDB_NAME:-scene_flow_pretrain_waymo_gb64_los_v6}"
...
if [[ "${GRAD_ACCUM_STEPS}" != "8" ]]; then
    echo "[错误] A2 单机实验固定 GRAD_ACCUM_STEPS=8。" >&2
    exit 1
fi
...
if [[ -n "${RESUME_PATH}" && -z "${WANDB_RUN_ID}" ]]; then
    echo "[错误] LOS resume 必须提供原 W&B run id。" >&2
    exit 1
fi
...
if (( GLOBAL_BATCH_SIZE != EXPECTED_GLOBAL_BATCH_SIZE )); then
    echo "[错误] global batch size 配置不符合 A2 单机合同：" >&2
...
setup_common_env() {
    ...
    # Match pretrain_single_node.sh's local NCCL contract; multi-node PPU
    # bootstrap settings do not apply on one host.
...
if missing:
    raise SystemExit(f"[错误] LOS resume 只接受完整训练 checkpoint；缺少 {missing}")
version = payload.get("pretrain_resume_contract_version")
if version != "layout_v2_pretrain_resume_v3":
    raise SystemExit(f"[错误] LOS resume contract 版本不匹配：{version!r}")
contract = payload.get("world_feedback_contract")
if not isinstance(contract, dict) or contract.get("profile") != "latent_only":
    profile = contract.get("profile") if isinstance(contract, dict) else None
    raise SystemExit(f"[错误] 明确拒绝 Full/旧 checkpoint；world_feedback_profile={profile!r}")
if contract.get("schema") != "world_feedback_contract_v1" or contract.get("compute_matched") is not True:
    raise SystemExit("[错误] LOS checkpoint 缺少有效的 compute-matched world-feedback contract")
expected_effective = {
    "lambda_rgb_render": 0.0,
    "lambda_level_consistency": 0.0,
    "lambda_head_consistency": 0.0,
}
if contract.get("effective") != expected_effective:
    raise SystemExit("[错误] LOS checkpoint effective world-feedback 权重不为全零")
config = payload.get("scene_flow_config")
scene_profile = config.get("scene_units_profile", "generated") if isinstance(config, dict) else "generated"
if scene_profile != "generated":
    raise SystemExit(f"[错误] 明确拒绝 FSU checkpoint；scene_units_profile={scene_profile!r}")
step = int(payload["step"])
if step != expected_step:
    raise SystemExit(f"[错误] checkpoint step={step} 与 EXPECTED_STEP={expected_step} 不一致")
PY
}
...
cd "${PROJECT_ROOT}"
build_train_args
LAUNCH_LOG="${LAUNCH_LOG_DIR}/single_node_los.log"

echo "============================================================"
echo "A2 Latent-Only Supervision 单机 8-GPU 预训练"
