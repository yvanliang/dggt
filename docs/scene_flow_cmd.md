...
* validation 图像会保存到 `logs/scene_flow_pretrain/validation/step_xxxxxx/`；默认包含 `generated_raw_3dgs_rgb__cfg*.jpg`（纯噪声经 SceneFlow 采样、tokenizer decode，并用 decoded token 派生的 DGGT `camera_head/depth_head/instance_head/gs_head` 输出；sky/non-sky mask 和 sky background 按 DGGT 原始 `inference.py` mode=2 对齐，使用 GT sky mask + `sky_model(images, extrinsic, intrinsic)`）、`dggt_clean_3dgs_rgb.jpg`（原始 DGGT clean 3DGS render）、`input_rgb_gt.jpg`、latent PCA、误差图和 mask 图。
