...
case "${MODE}" in
    ...
    --launch)
        ...
        ssh "${SSH_OPTS[@]}" \
            ...
            "test -r '${SCRIPT_PATH}' && NETWORK_MODE='${NETWORK_MODE}' bash '${SCRIPT_PATH}' --check-worker"

        echo
        echo "=== 3/4 启动副节点 rank ${WORKER_RANK} ==="
        ssh "${SSH_OPTS[@]}" \
            "${WORKER_USER}@${WORKER_HOST}" \
            "NETWORK_MODE='${NETWORK_MODE}' bash '${SCRIPT_PATH}' --worker"
        ...
        if ! launch_node \
            ...
            ssh "${SSH_OPTS[@]}" \
                "${WORKER_USER}@${WORKER_HOST}" \
                "NETWORK_MODE='${NETWORK_MODE}' bash '${SCRIPT_PATH}' --stop-worker" || true
            exit 1
        fi
