...
usage() {
    cat <<'EOF'
用法：
  bash pretrain_single_node_islo_ablation.sh

Image-Space Layout Only（A3/ISLO）单机训练：
  ...
  - scene gauge、global gauge context、A shared-attention tokens、HDS、sky、mask、
    REPA、flow 等其余路径和损失全部保留；
  - 除单机拓扑外，global batch、训练、验证与 W&B 参数均对齐两机 PPU ISLO launcher。
EOF
}
...
if (( $# != 0 )); then
    echo "[错误] A3/ISLO 单机 launcher 不接受位置参数。" >&2
    usage >&2
    exit 2
fi
...
# A3 is orthogonal to A1/A2: preserve Full scene units and world feedback,
# changing only the early/late layout injection architecture switches.
...
LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-${PROJECT_ROOT}/logs/single_node_islo_v6_launch}"

# ============================================================
# Training contract requested for A3 single-node runs:
...
if [[ "${GRAD_ACCUM_STEPS}" != "8" ]]; then
    echo "[错误] A3 单机实验固定 GRAD_ACCUM_STEPS=8。" >&2
    exit 1
fi
...
if (( GLOBAL_BATCH_SIZE != EXPECTED_GLOBAL_BATCH_SIZE )); then
    echo "[错误] global batch size 配置不符合 A3 单机合同：" >&2
...
cd "${PROJECT_ROOT}"
build_train_args
LAUNCH_LOG="${LAUNCH_LOG_DIR}/single_node_islo.log"

echo "============================================================"
echo "A3 Image-Space Layout Only 单机 8-GPU 预训练"
