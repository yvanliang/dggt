...
usage() {
    cat <<'EOF'
用法：
  bash pretrain_ppu_one_node_fsu_dlc.sh
  bash pretrain_ppu_one_node_fsu_dlc.sh --resume CHECKPOINT EXPECTED_STEP WANDB_RUN_ID
...
Fixed Scene Units（fsu）消融：
  ...
  - 固定使用 1 节点 × 16 PPU、每 PPU batch 1、gradient accumulation 4；
  - 保留公共 launcher 的 global batch 64 和 50-step validation。
EOF
}
...
if [[ ! -r "${COMMON_LAUNCHER}" ]]; then
    echo "[错误] 找不到公共 PPU launcher：${COMMON_LAUNCHER}" >&2
    exit 1
fi
...
export LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-/mnt/workspace/dggt/logs/ppu_dlc_one_node_fsu_v6_launch}"
