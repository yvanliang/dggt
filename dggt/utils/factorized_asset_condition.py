...
def sample_canonical_tokens(
    ...
    *,
    ...
    for row in range(n):
        ...
        out_uv[row, :count] = uv_all.index_select(0, selected)
...
def project_anchor_boxes_to_patch_bboxes(
    ...
    depth = corners_cam[..., 2]
    positive = depth > 1.0e-5
    proj_h = torch.matmul(Ks[:, None, :, None], corners_cam.unsqueeze(-1)).squeeze(-1)
    xy = proj_h[..., :2] / depth.clamp_min(1.0e-5).unsqueeze(-1)
    inf = torch.full_like(xy[..., 0], float("inf"))
    ninf = torch.full_like(xy[..., 0], float("-inf"))
    x0 = torch.where(positive, xy[..., 0], inf).amin(dim=-1)
    y0 = torch.where(positive, xy[..., 1], inf).amin(dim=-1)
    x1 = torch.where(positive, xy[..., 0], ninf).amax(dim=-1)
    y1 = torch.where(positive, xy[..., 1], ninf).amax(dim=-1)
    has_depth = positive.any(dim=-1)
    height = hw[:, None, :, 0]
    width = hw[:, None, :, 1]
    in_frustum = (
        track_valid
